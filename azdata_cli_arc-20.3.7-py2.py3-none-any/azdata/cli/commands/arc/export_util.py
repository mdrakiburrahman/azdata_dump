...
import os
import sys
from azdata.cli.commands.sqlmi.constants import SQLMI_LICENSE_TYPE_BASE_PRICE, SQLMI_LICENSE_TYPE_BASE_PRICE_AZURE, SQLMI_LICENSE_TYPE_LICENSE_INCLUDED, SQLMI_LICENSE_TYPE_LICENSE_INCLUDED_AZURE, SQLMI_TIER_BUSINESS_CRITICAL_ALL, SQLMI_TIER_BUSINESS_CRITICAL_AZURE, SQLMI_TIER_GENERAL_PURPOSE_ALL, SQLMI_TIER_GENERAL_PURPOSE_AZURE
...
from azdata.cli.commands.arc import constants, export_instance_properties as instance_properties
from azdata.cli.core.exceptions import CliError, RequestTimeoutError, ServerError, http_status_codes
from azdata.cli.core.deploy import display
from azdata.cli.core.serialization import SanitizerRule
...
ERROR_IGNORE = 'should not be older than 30 minutes and not more than 4 minutes in the future'
"""
...
def metrics_upload(metrics):
    ...
    if metrics is None:
        ...
        for data in metrics:
            ...
            filtered_metrics = list(filter(lambda metric: (current_time - datetime.datetime.strptime(metric["time"], "%Y-%m-%dT%H:%M:%SZ")).total_seconds()/60 <= 30, metrics))
            if filtered_metrics is None or not filtered_metrics:
                print("The metrics data are older than 30 minutes for {}, please export and upload again.".format(resource_id_value))
                continue
            log.info("Metrics data in file has {} records. {} are in last 30 mins".format(len(metrics), len(filtered_metrics)))
            ...
            res = util.retry(
                lambda: _post_metrics(url, body=ndjson.dumps(filtered_metrics), headers=headers),
...
def _parse_resource(resource):
    ...
    if len(parts) == 2:
        ...
        raise Exception(
            "Cannot parse resource '{}'. Acceptable formats are 'namespace_name' or 'name'.".format(
                name))
...
def _set_header():
    ...
    headers = {'Authorization': 'Bearer ' + acquire_token(azure_constants.AZURE_METRICS_SCOPE),
               'Content-Type': 'application/x-ndjson'}
    return headers


def _set_url(location, resource_id):
    url = "https://{}.monitoring.azure.com{}/metrics".format(location,
                                                             resource_id)
    return url
    ...
    if resource_uri is not None:
        ...
        for log in logs:
            result = _convert_to_logs_format(
                ...
                azure_constants.RESOURCE_TYPE_FOR_KIND[resource[instance_properties.KIND]], 
                resource_uri)
...
def logs_upload(logs, customer_id, shared_key):
    ...
    for log in logs:
        unzip = str(zlib.decompress(base64.b64decode(log['logs']), -zlib.MAX_WBITS), 'utf-8')
        log_table_name = log['instance_type'] + '_logs'
        _post_logs_to_logs_analytics(customer_id, 
                                    shared_key,
                                    log['instance_name'],
                                    unzip,
                                    log_table_name,
...
def _get_log_workspace_credentials_from_env(client):
    ...
    if get_workspace_from_env:
        ...
        while not succeed:
            display(
                'Please provide Azure Log Analytics workspace information')
            ...
            try:
                ...
                client.stderr(
                    'Can\'t upload to the workspace: "{} Exception: {}". Please try again'.format(
                        customer_id, e))
                display('\n')
...
def _store_log_workspace_credentials_in_env(customer_id, shared_key):
    ...
    if customer_id and shared_key:
        ...
        raise ValueError("Failed to store one or more of the following: {}".format(list(WORKSPACE_ENV_KEYS)))


def _build_log_request_header(customer_id, shared_key, content_length, log_type,
                              resource_uri):
    ...
    signature = _build_log_request_signature(customer_id, shared_key,
...
def _build_log_request_uri(customer_id):
    return 'https://' + customer_id + '.ods.opinsights.azure.com' + \
        azure_constants.API_LOG + '?api-version=2016-04-01'


# Build the API signature
def _build_log_request_signature(customer_id, shared_key, date, content_length,
...
def _test_log_workspace_key(customer_id, shared_key):
    ...
    headers = _build_log_request_header(customer_id, shared_key, len(body),
                                        table_name, uri)
    succeed = False
    try:
...
def _post_logs_to_logs_analytics(customer_id, shared_key, instance_name,
                                 body, log_type, instance_type, resource_uri):
    uri = _build_log_request_uri(customer_id)
    headers = _build_log_request_header(customer_id, shared_key, len(body),
                                        log_type, resource_uri)

    response = _requests_retry_session().post(uri, data=body, headers=headers)
    if 200 <= response.status_code <= 299:
        display(
            '\tSuccessfully upload "{}" records for resource type "{}", instance: "{}\'s" log to table: "{}" '.format(
                len(json.loads(body)),
                instance_type,
                instance_name,
                log_type))
        ...
        raise Exception(
            '\tFail to upload "{}" to table: "{}" with status code: "{}" and error msg: "{}"'.format(
                instance_name,
                log_type,
...
def _requests_retry_session(
    ...
    :arg:
      ...
      - status_forcelist: server error code list
        ...
        https://docs.microsoft.com/en-us/azure/azure-monitor/platform/data-collector-api#return-codes
...
def get_export_timestamp(export_type):
    ...
    default_start_time = datetime.datetime.utcnow() - datetime.timedelta(minutes=constants.DEFAULT_QUERY_WINDOW[export_type])
...
def update_upload_status_file(export_type, data_timestamp):
    ...
    """
    try:
        upload_status_file_path = _get_upload_status_file_path(UPLOAD_STATUS_FILENAME)
        with open(upload_status_file_path, 'r+', encoding='utf-8') as upload_status_file:
            upload_status_json = json.load(upload_status_file)

            if data_timestamp:
                upload_status_json[export_type]['data_timestamp'] = data_timestamp
                upload_status_json[export_type]['upload_timestamp'] = datetime.datetime.utcnow().isoformat(sep=' ', timespec='milliseconds')

            upload_status_file.seek(0)
            json.dump(upload_status_json, upload_status_file, indent=4)
            upload_status_file.truncate
            print("Update status file {0} data_timestamp to {1}".format(export_type, data_timestamp))
...
def _get_upload_status_file_path(filename):
    """
    Returns the upload status file's path and schema_valid, which represents whether
    or not the input file was valid. If the input file did not follow the expected schema, 
    then the file will be rewritten with default values.
    """
    from datetime import datetime, timedelta

    config_dir = os.path.expanduser(Configuration().CLI_CONFIG_DIR)
    status_file = os.path.join(config_dir, '{}-{}'.format(Configuration().CLI_NAME, filename))
    ...
    log.info("Upload status file={0} status_file_exists={1}, schema_valid={2}".format(status_file, status_file_exists, schema_valid))
    ...
    if not status_file_exists or not schema_valid:
        ...
        new_upload_status_file_content = {
            "metrics": {
                "upload_timestamp": upload_timestamp.isoformat(sep=' ', timespec='milliseconds'),
                "data_timestamp": (upload_timestamp - timedelta(minutes=constants.DEFAULT_METRIC_QUERY_WINDOW_IN_MINUTE)).isoformat(sep=' ', timespec='milliseconds')
            },
            "logs": {
                "upload_timestamp": upload_timestamp.isoformat(sep=' ', timespec='milliseconds'),
                "data_timestamp": (upload_timestamp - timedelta(days=constants.DEFAULT_LOG_QUERY_WINDOW_IN_DAY)).isoformat(sep=' ', timespec='milliseconds')
            },
            "usage": {
                "upload_timestamp": upload_timestamp.isoformat(sep=' ', timespec='milliseconds'),
                "data_timestamp": (upload_timestamp - timedelta(days=constants.DEFAULT_USAGE_QUERY_WINDOW_IN_DAY)).isoformat(sep=' ', timespec='milliseconds')
            }
        }

        with open(status_file, mode='w', encoding='utf-8') as new_upload_status_file:
            json.dump(new_upload_status_file_content, new_upload_status_file, indent=4)
...
def validate_upload_status_file(path):
    ...
    Returns whether or not the input upload status file follows the expected JSON schema.
    """
    try:
...
UPLOAD_STATUS_FILE_SCHEMA = {
        ...
        "properties": {
            "metrics": {
                "type": "object",
                "properties": {
                    "upload_timestamp": {"type": "string", "maxLength": 23, "minLength": 23},
                    "data_timestamp": {"type": "string", "maxLength": 23, "minLength": 23},
                },
                "required": [
                    "upload_timestamp",
                    "data_timestamp"
...
def format_sqlmi_tier_for_azure(tier):
    """
    Given a tier, return it in a format expected by Azure (e.g: translate 'bc' to 'BusinessCritical')
    """
...
def format_sqlmi_license_type_for_azure(license_type):
    """
    Given a license type, return it in a format expected by Azure (e.g: translate 'licenseincluded' to 'LicenseIncluded')
    """
